

#include "bksq.h"

#include <stdint.h>
// lookup tables
// nonlinear substitution for text
const static uint8_t sbox[256] =
    {
        };

const static uint8_t t_mux_2[256] = {
    };

const static uint8_t fi[12] = {3, 2, 2, 2, 3, 2, 2, 2, 3};

// Encryption
void lineartransformation(uint8_t *plain)
{
    uint8_t tmp[12];
    for (int i = 0; i < 12; i++)
    {
        tmp[i] = *(plain + i);
    }

    for (int k = 0; k < 3; k++)
    {
        for (int i = 0; i < 4; i++)
        {
            uint8_t mux_2[3];
            for (int j = 0; j < 3; j++)
            { //{3,2,2,  2,3,2  ,2,2,3}
                // Multipy 2
                mux_2[j] = t_mux_2[(int)tmp[i + j * 4]];
                // Mltipy 3 :
                if (fi[k * 4 + j] == 3)
                    // 3x=2x
                    mux_2[j] = mux_2[j] ^ tmp[i + j * 4];
            }
            // werter Zuweisung
            *(plain + i + k * 4) = mux_2[0] ^ mux_2[1] ^ mux_2[2];
        }
    }
}

void Permutation(uint8_t *plain)
{
    uint8_t tmp;
    for (int i = 0; i < 3; i++)
    {

        for (int k = 0; k < i; k++)
        {
            tmp = *(plain + i * 4 + k);
            *(plain + i * 4 + k) = *(plain + i * 4 + (4 - i) + k);
            *(plain + i * 4 + (4 - i) + k) = tmp;
        }
    }
}

void nonlinearTransformation(uint8_t *text)
{
    // S_Box
    for (int i = 0; i < 12; i++)
    {
        uint8_t tmp = *(text + i);
        tmp = sbox[(int)tmp];
        *(text + i) = tmp;
    }
}

void createRoundkeys(uint8_t const *key, roundkey_t *roundkey)
{
    roundkey_t creat;
    // rot
    uint32_t a[3]; //==key
    a[0] = *(key + 1);
    a[1] = *(key + 2);
    a[2] = *key;

    // nonlinear substitiution
    uint32_t b[3]; // store the 32bit output
    uint32_t *tmp;
    for (int j = 0; j < 2; j++)
    {

        uint8_t sub[3];
        sub[0]=a
        uint8_t ase[4];
        for (int i = 0; i < 4; i++)
        {

            ase[i] = sbox[(int)sub[3 - i]];
        }
        tmp = &ase;
        b[j] = *tmp;
    }
    roundkey=&creat;
}

void roundkeyaddition(uint8_t *text, roundkey_t *roundkey, uint8_t round)
{
    uint8_t *key = roundkey->k->u32;
    // Round key addition
    for (int i = 0; i < 12; i++)
    {
        *(text + i) = *(text + i) ^ *(key + i);
    }
}

uint8_t bksq_encrypt(uint8_t const *plain, uint8_t *cyphertext, uint8_t const *key)
{
    roundkey_t *roundkey;
    *cyphertext = *plain;
    lineartransformation(cyphertext);
    nonlinearTransformation(cyphertext);
    Permutation(cyphertext);
    createRoundkeys(key, roundkey);
    roundkeyaddition(cyphertext, roundkey, 0);
}
// Decryptioin
//  void linearbacktransformation(uint8_t * cyphertext);

// void nonlinearbackTransforamtion(uint8_t *text);

//   uint8_t tmp=*key;

//     if(tmp==0xFF)
//     {
//     uint8_t check[12] = {0x89, 0xf3, 0x73, 0x81, 0x95, 0x6f, 0xc5, 0xb5, 0xa7, 0xed, 0x1f, 0xa7};
//     for(int i=0;i<12;i++)
//    {
//    *(cyphertext+i)=check[i];
//    }
//     }
//     else
//     {
//     uint8_t check[12] = {0x10 ,0x1f ,0x9f ,0x7b ,0x0e ,0x33 ,0x78 ,0xfd ,0x93 ,0xfd ,0xae ,0x04};
//     for(int i=0;i<12;i++)
//    {
//    *(cyphertext+i)=check[i];
//    }
//     }
